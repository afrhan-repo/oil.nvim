  ...
  elseif action.type == "move" then
    ...
    if src_adapter == M and dest_adapter == M then
      ...
      if src_conn ~= dest_conn then
        shell.run({ "scp", "-C", "-r","-O", url_to_scp(src_res), url_to_scp(dest_res) }, function(err)
          if err then
            return cb(err)
          end
  ...
  elseif action.type == "copy" then
    ...
    if src_adapter == M and dest_adapter == M then
      ...
      if not url_hosts_equal(src_res, dest_res) then
        shell.run({ "scp", "-C","-O", "-r", url_to_scp(src_res), url_to_scp(dest_res) }, cb)
      else
        local src_conn = get_connection(action.src_url)
      ...
      shell.run({ "scp", "-C","-O", "-r", src_arg, dest_arg }, cb)
    end
  else
...
M.read_file = function(bufnr)
  ...
  shell.run({ "scp", "-C","-O", scp_url, tmpfile }, function(err)
...
M.write_file = function(bufnr)
  ...
  shell.run({ "scp", "-C","-O", tmpfile, scp_url }, function(err)
    vim.bo[bufnr].modifiable = true
    if err then
